...
const DRILL_SLOTS = {
  1: [{ hour: 8, label: '8:00–9:00' }],
  2: [{ hour: 7, label: '7:00–8:00' }, { hour: 8, label: '8:00–9:00' }],
  3: [{ hour: 8, label: '8:00–9:00' }],
  4: [{ hour: 7, label: '7:00–8:00' }, { hour: 8, label: '8:00–9:00' }],
  6: [{ hour: 7, label: '7:00–8:00' }, { hour: 8, label: '8:00–9:00' }],
}
...
export default function Drills() {
  ...
  const [drillType, setDrillType] = useState('private')
  ...
  // Confirm dialogs
  const [confirmPreSystem, setConfirmPreSystem] = useState(null) // drill id
  const [confirmCheckin,   setConfirmCheckin]   = useState(null) // drill id
  ...
  function notify(msg) { setNotif(msg); setTimeout(() => setNotif(''), 4000) }
  ...
  function handleDateChange(dateStr) {
    ...
    setAvailableSlots(getSlotsForDate(dateStr))
  }
  ...
  // ── Check-in: confirma llegada, cobra el paquete, descuenta 1 clase ──────
  async function handleCheckin(id) {
    ...
    // Si es paquete, descontar 1 clase en el check-in
    if (drill.package_type && drill.package_type !== 'single' && drill.classes_remaining > 0) {
      updates.classes_remaining = drill.classes_remaining - 1
    }

    await supabase.from('drills').update(updates).eq('id', id)
    setDrills(prev => prev.map(d => d.id === id ? { ...d, ...updates } : d))
    setConfirmCheckin(null)

    const msg = updates.classes_remaining !== undefined
      ? `✅ Check-in confirmado — clase descontada, quedan ${updates.classes_remaining} clases · $${drill.total_mxn} MXN sumados a ventas`
      : `✅ Check-in confirmado · $${drill.total_mxn} MXN sumados a ventas`
    notify(msg)
  }

  // ── Pago previo al sistema: marca como pagado pero NO suma a ventas ───────
  async function handlePreSystem(id) {
    const updates = {
      status:             'confirmed',
      revenue_collected:  true,
      pre_system_payment: true,
    }
    await supabase.from('drills').update(updates).eq('id', id)
    setDrills(prev => prev.map(d => d.id === id ? { ...d, ...updates } : d))
    setConfirmPreSystem(null)
    notify('📋 Pago previo registrado — solo se lleva el control de clases, sin sumar a ventas')
  }

  // ── Descuento manual de clase ─────────────────────────────────────────────
  async function deductClass(id) {
    ...
    notify(`📚 Clase descontada manualmente — quedan ${newRemaining} clases`)
  }

  async function cancelDrill(id) {
    await supabase.from('drills').update({ status: 'cancelled' }).eq('id', id)
    setDrills(prev => prev.map(d => d.id === id ? { ...d, status: 'cancelled' } : d))
    notify('❌ Drill cancelado')
  }
  ...
  // Metrics — solo revenue_collected y NO pre_system
  const active          = drills.filter(d => d.status !== 'cancelled')
  const collectedRev    = active.filter(d => d.revenue_collected && !d.pre_system_payment)
  const totalRevMXN     = collectedRev.reduce((s, d) => s + Number(d.total_mxn || 0), 0)
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontFamily: 'var(--font-cond)', fontSize: 28, fontWeight: 800, letterSpacing: 1 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 8, marginBottom: 16 }}>
        {[
          ...
          { label: 'COBRADO',        val: fmtMXN(totalRevMXN) },
          ...
          {showForm && (
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                {[{ id: 'private', label: '🎯 Privada' }, { id: 'collective', label: '📦 Paquetes' }].map(t => (
                  <button key={t.id} onClick={() => setDrillType(t.id)} className="btn btn-sm"
                    style={{ background: drillType === t.id ? 'var(--g)' : 'var(--sf)', color: drillType === t.id ? '#fff' : 'var(--mt)', border: `1px solid ${drillType === t.id ? 'var(--g)' : 'var(--br)'}`, fontWeight: drillType === t.id ? 700 : 400 }}>
                    {t.label}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                <div>
                  ...
                  <input type="date" className="form-input" value={form.date} min={todayStr()} onChange={e => handleDateChange(e.target.value)} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                <div>
                  ...
                  <input className="form-input" placeholder="Juan García" value={form.clientName} onChange={e => setForm(f => ({ ...f, clientName: e.target.value }))} />
                </div>
                <div>
                  <label className="form-label">Teléfono (opcional)</label>
                  <input className="form-input" placeholder="+52 998 000 0000" value={form.clientPhone} onChange={e => setForm(f => ({ ...f, clientPhone: e.target.value }))} />
                </div>
              </div>

              <div style={{ marginBottom: 10 }}>
                <label className="form-label">Notas (opcional)</label>
                <input className="form-input" placeholder="Nivel principiante, objetivos..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
              </div>
              ...
              {form.date && form.hour && (
                <div style={{ background: 'var(--glight)', border: '1px solid var(--gd)', borderRadius: 6, padding: '8px 12px', marginBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, color: 'var(--g)' }}>
                    {drillType === 'private'
                      ? `Clase privada · ${form.people}p`
                      : `Clase colectiva · ${form.packageType === 'single' ? '1 clase' : form.packageType === 'pack8' ? '8 clases' : '12 clases'}`}
                  </span>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn btn-green btn-sm" onClick={saveDrill} disabled={saving}>{saving ? 'Guardando…' : 'Agendar Drill'}</button>
          ...
          {filtered.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filtered.map(d => {
                ...
                const isPending   = d.status === 'reserved'
                const isConfirmed = d.status === 'confirmed'
                const isPack      = d.package_type && d.package_type !== 'single'
                  ...
                  <div key={d.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                      <div style={{ flex: 1, minWidth: 200 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                          <div style={{ fontFamily: 'var(--font-cond)', fontSize: 16, fontWeight: 700 }}>{d.client_name}</div>
                          ...
                          {/* Pago previo al sistema */}
                          {d.pre_system_payment && (
                            <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: '#1e1e1e', color: 'var(--mt)', border: '1px solid var(--br)' }}>
                              💳 Pago previo
                            </span>
                          )}

                          {/* Revenue cobrado */}
                          {d.revenue_collected && !d.pre_system_payment && (
                            <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: '#0d2010', color: 'var(--g)', border: '1px solid var(--gd)' }}>
                              ✅ Cobrado
                            </span>
                          )}
                        </div>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        ...
                        {d.pre_system_payment && (
                          <div style={{ fontSize: 10, color: 'var(--mt)', marginTop: 2 }}>no suma a ventas</div>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, paddingTop: 8, borderTop: '1px solid var(--br)', flexWrap: 'wrap', gap: 6 }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {/* CHECK-IN — solo si está reserved y no hay pago previo confirmado */}
                        {isPending && (
                          <>
                            {confirmCheckin === d.id ? (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#0d2010', border: '1px solid var(--g)', borderRadius: 7, padding: '6px 10px' }}>
                                <span style={{ fontSize: 11, color: 'var(--g)' }}>
                                  ¿Confirmar llegada?{isPack ? ` Se descontará 1 clase (quedan ${d.classes_remaining - 1})` : ''} · {fmtMXN(d.total_mxn)} a ventas
                                </span>
                                <button className="btn btn-green btn-sm" onClick={() => handleCheckin(d.id)} style={{ fontSize: 11 }}>Sí, check-in</button>
                                <button className="btn btn-ghost btn-sm" onClick={() => setConfirmCheckin(null)} style={{ fontSize: 11 }}>No</button>
                              </div>
                            ) : (
                              <button className="btn btn-sm" onClick={() => setConfirmCheckin(d.id)}
                            ...
                            {/* PAGO PREVIO AL SISTEMA */}
                            {confirmPreSystem === d.id ? (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#1e1e1e', border: '1px solid var(--br)', borderRadius: 7, padding: '6px 10px' }}>
                                <span style={{ fontSize: 11, color: 'var(--mt)' }}>¿Este cliente pagó antes de tener sistema? No sumará a ventas.</span>
                                <button className="btn btn-sm" onClick={() => handlePreSystem(d.id)}
                                  style={{ fontSize: 11, background: '#2a2a2a', color: 'var(--mt)', border: '1px solid var(--br)' }}>
                                  Sí, confirmar
                                </button>
                                <button className="btn btn-ghost btn-sm" onClick={() => setConfirmPreSystem(null)} style={{ fontSize: 11 }}>No</button>
                              </div>
                            ) : (
                              <button className="btn btn-sm" onClick={() => setConfirmPreSystem(d.id)}
                                style={{ background: '#1e1e1e', color: 'var(--mt)', border: '1px solid var(--br)', fontSize: 12 }}>
                                💳 Pago previo al sistema
                              </button>
                            )}

                            <button className="btn btn-sm" onClick={() => { if (window.confirm('¿Cancelar este drill?')) cancelDrill(d.id) }}
                        ...
                        {/* Descuento manual — solo si está confirmado con paquete y quedan clases */}
                        {isConfirmed && isPack && d.classes_remaining > 0 && (
      ...
      {tab === 'paquetes' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--mt)' }}>Selecciona un cliente para agendar su próxima clase</div>
          </div>
          ...
          {packClients.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {packClients.map(d => {
                ...
                const usedClasses  = totalClasses - (d.classes_remaining ?? totalClasses)
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                      <div style={{ flex: 1, minWidth: 200 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2, flexWrap: 'wrap' }}>
                          <div style={{ fontFamily: 'var(--font-cond)', fontSize: 16, fontWeight: 700 }}>{d.client_name}</div>
                          {d.pre_system_payment && (
                            <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 4, background: '#1e1e1e', color: 'var(--mt)', border: '1px solid var(--br)' }}>
                              💳 Pago previo
                            </span>
                          )}
                        </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                        <div style={{
                          background: 'var(--sf)', borderRadius: 10, padding: '8px 20px', textAlign: 'center', minWidth: 80,
                          border: `2px solid ${isEmpty ? 'var(--rd)' : isLow ? 'var(--am)' : 'var(--gd)'}`
                        }}>
                          <div style={{ fontFamily: 'var(--font-cond)', fontSize: 32, fontWeight: 800, lineHeight: 1, color: isEmpty ? 'var(--rd)' : isLow ? 'var(--am)' : 'var(--g)' }}>
                            {d.classes_remaining}
                          </div>
                        ...
                        {!isEmpty && (
                          <button className="btn btn-green btn-sm" style={{ width: '100%', fontSize: 11 }}
                            onClick={() => {
                              setDrillType('collective')
                              setForm(f => ({ ...f, clientName: d.client_name, clientPhone: d.client_phone || '', packageType: d.package_type, date: '', hour: '' }))
                              setShowForm(true)
                              setTab('reservas')
                        ...
                        {isEmpty && (
                          <div style={{ fontSize: 11, color: 'var(--rd)', fontWeight: 600 }}>Sin clases</div>
                        )}
                      </div>
                    </div>
